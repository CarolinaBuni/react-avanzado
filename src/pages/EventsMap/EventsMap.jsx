...
//     console.log("Updated focusedEventId to:", eventId);
...
import PopupManager from '../../components/PopupManager/PopupManager';
import { eventsReducer, INITIAL_STATE } from '../../utils/reducer';
import { setMap, showAllEvents } from '../../utils/actions';
import useFavoritesManager from '../../Hooks/useFavoritesManager';
import { usePopup } from '../../context/PopupContext';
import Filters from '../../components/Filters/Filters';
import UpcomingEventsModal from '../../components/UpcomingEventsModal/UpcomingEventsModal';
...
const EventsMap = () => {
  ...
  const handleToggleMarkers = useCallback((eventId = null) => {
    if (eventId !== null) {
      // Seleccionando un evento específico
      const eventIdStr = String(eventId); // Convertir a cadena
      setFocusedEventId(eventIdStr); // Actualiza el evento enfocado
      console.log("Focused on eventId:", eventIdStr);

      // if (state.showMarkers) {
      //   dispatch({ type: 'TOGGLE_MARKERS' }); // Desactivar showMarkers si estaba activo
      // }
      ...
      // Alternar la visibilidad de todos los marcadores
      dispatch({ type: 'TOGGLE_MARKERS' });
    if (!state.showMarkers) {
      dispatch({ type: 'SHOW_ALL_EVENTS' });
      setFilteredLocations(locations);
  ...
  }, [locations, state.map, state.showMarkers, dispatch]);
